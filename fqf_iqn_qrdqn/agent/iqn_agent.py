...
from morl import memories, models, learners
...
class IQNAgent(BaseAgent):
    ...
    def __init__(self, env, test_env, log_dir, num_steps=5*(10**7),
        ...
        if use_morl():
            ...
            self.online_net = models.DiscreteIQN.nature_dqn(
                state_channels=4,
                state_height=84,
                state_width=84,
                n_actions=self.num_actions,
                n_action_quantiles=K,
                n_cosines=num_cosines,
                device="cuda",
            )
            self.target_net = extu.make_target_net(self.online_net)
            self.morl_learner = learners.IQNLearner(
    ...
    def learn(self):
        ...
        if use_morl():
            ...
            # tensor_exp.states = tensor_exp.states.float().div(255)
            # tensor_exp.next_states = tensor_exp.next_states.float().div(255)
            quantile_loss, errors = self.morl_learner._compute_loss(tensor_exp)
        else:
