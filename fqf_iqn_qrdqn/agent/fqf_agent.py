...
class FQFAgent(BaseAgent):
    def __init__(
        ...
        if use_morl():
            self.morl_agent = factories.construct_from_dict_config(
                OmegaConf.load(
                    "/home/azureuser/workspace/personal/morl/morl/config_templates/trainer_fqf_atari.yml"
                ),
                api.Agent,
            )
            self.morl_learner = self.morl_agent.learner
            self.morl_policy = self.morl_agent.policy
            self.morl_memory = self.morl_learner.memory
        ...
        # )
        self.fraction_optim = RMSprop(
            self.online_net.fpn_parameters(),
            lr=fraction_lr,
            alpha=0.95,
            eps=0.00001,
        )
        ...
        self.quantile_optim = Adam(
            self.online_net.quantile_parameters(),
            lr=quantile_lr,
            eps=1e-2 / batch_size,
        )
    ...
    def update_target(self):
        # self.target_net.dqn_net.load_state_dict(self.online_net.dqn_net.state_dict())
        ...
        # )
        extu.hard_model_update_(self.online_net, self.target_net)
    ...
    def learn(self):
        ...
        if use_morl():
            ...
            states = tensor_exp.states.float().div(255)
            actions = tensor_exp.actions.long()
            next_states = tensor_exp.next_states.float().div(255)
            rewards = tensor_exp.rewards
            dones = tensor_exp.dones.float()
            weights = None
        elif self.use_per:
            (
                states,
                ...
                next_states,
                dones,
            ), weights = self.memory.sample(self.batch_size)
        else:
        ...
        state_embeddings = self.online_net.embeddings(states)
        ...
        # )
        taus, tau_hats, entropies = self.online_net.fractions(
            state_embeddings=state_embeddings.detach()
        )
        ...
        #     actions)
        current_sa_quantile_hats = extu.evaluate_quantiles_at_actions(
            self.online_net.quantiles(
                taus=tau_hats, state_embeddings=state_embeddings
            ),
            actions,
        )
        ...
        fraction_loss = self.calculate_fraction_loss(
            state_embeddings.detach(),
            current_sa_quantile_hats.detach(),
            taus,
            actions,
            weights,
        )
        ...
        #     quantile_loss,
        #     networks=[
        #         self.online_net.dqn_net,
        #         self.online_net.cosine_net,
        #         self.online_net.quantile_net,
        #     ],
        #     retain_graph=False,
        #     grad_cliping=self.grad_cliping,
        # )
        extu.step_optimizer_(
            loss_=fraction_loss + entropy_loss,
            optimizer_=self.fraction_optim,
            retain_graph=True,
            clip=self.grad_cliping,
            parameters=self.online_net.fpn_parameters(),
        )
        extu.step_optimizer_(
            loss_=quantile_loss,
            optimizer_=self.quantile_optim,
            clip=self.grad_cliping,
            parameters=self.online_net.quantile_parameters(),
        )
    ...
    def calculate_fraction_loss(
        self, state_embeddings, sa_quantile_hats, taus, actions, weights
    ):
        ...
        with torch.no_grad():
            ...
            #     actions,
            # )
            sa_quantiles = extu.evaluate_quantiles_at_actions(
                self.online_net.quantiles(
                    taus=taus[:, 1:-1], state_embeddings=state_embeddings
                ),
                actions,
        ...
        if weights is not None:
            fraction_loss = (
                ((gradient_of_taus * taus[:, 1:-1]).sum(dim=1, keepdim=True)) * weights
            ).mean()
        else:
    ...
    def calculate_quantile_loss(
        ...
        with torch.no_grad():
            ...
            if self.double_q_learning:
                ...
                # )
                fqf_outputs = self.target_net(
                    states=next_states,
                    # fraction_net=self.online_net.fraction_net,
                    fpn=self.online_net.fraction_proposal_net,
                )
            next_state_embeddings = fqf_outputs.embeddings
            next_q = fqf_outputs.outputs
            # Calculate greedy actions.
            next_actions = torch.argmax(next_q, dim=1, keepdim=True)
            assert next_actions.shape == (self.batch_size, 1), next_actions.shape
            ...
            #     )
            if self.double_q_learning:
                next_state_embeddings = self.target_net.embeddings(
                    next_states
                )

            # Calculate quantile values of next states and actions at tau_hats.
            # next_sa_quantile_hats = extu.evaluate_quantiles_at_actions(
        ...
        quantile_huber_loss = extu.quantile_huber_loss(
            td_errors, tau_hats.unsqueeze(-1), weights, self.kappa
        )

        return (
            quantile_huber_loss,
            next_q.detach().mean().item(),
